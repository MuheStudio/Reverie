...
def test_bootstrap_failure_does_not_lock_root_initialised(
    ...
    """
    service_name = "PluginServerBootstrapFailureTest"
    monkeypatch.setenv("NEKO_PLUGIN_SERVICE_NAME", service_name)
    ...
    plogging = importlib.import_module("plugin.logging_config")
    plogging._root_initialised = False
    ...
    for h in logging.getLogger(f"N.E.K.O.{service_name}").handlers:
        h.flush()
    text = _read_logs(isolated_log_dir, service_name)
