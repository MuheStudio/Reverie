...
def pytest_configure(config: pytest.Config) -> None:
    for marker in (
        ...
        "plugin_integration: plugin integration-level test marker",
    ):
...
def _clear_leaked_running_loop(request: pytest.FixtureRequest):
    ...
    if "plugin_e2e" in request.node.keywords or {
        "browser",
        "context",
        "new_context",
        "page",
...
@pytest.fixture
def plugin_async_client(plugin_test_app: FastAPI) -> AsyncIterator[AsyncClient]:
