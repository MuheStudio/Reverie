...
except Exception:  # pragma: no cover - optional dependency in this workspace
    BeautifulSoup = None  # type: ignore[assignment]

try:
    from markdownify import markdownify as _markdownify_html  # type: ignore[import-untyped]
except Exception:  # pragma: no cover - optional dependency in this workspace
    _markdownify_html = None  # type: ignore[assignment]


def _fallback_html_to_markdown(value: str) -> str:
    text = re.sub(
    ...
    text = re.sub(r"</(p|div|section|article|main|li)>", "\n", text, flags=re.IGNORECASE)
    text = re.sub(r"<[^>]+>", "", text)
...
def _html_to_markdown(value: str) -> str:
    if _markdownify_html is not None:
        markdown = _markdownify_html(value, heading_style="ATX").strip()
        if markdown:
            return markdown
    return _fallback_html_to_markdown(value)
...
class MCPClient:
    ...
    def _normalize_html_payload(self, payload: object, *, source_url: str | None = None) -> Dict[str, object] | None:
        ...
        if BeautifulSoup is not None:
            ...
            markdown = _html_to_markdown(str(target))
        else:
            markdown = _fallback_html_to_markdown(html)
        if not markdown:
            return None
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, object], timeout: float = 60.0) -> Dict[str, object]:
        ...
        try:
            ...
            source_url = arguments.get("url") if isinstance(arguments.get("url"), str) else None
            normalized_payload = self._normalize_html_payload(payload, source_url=source_url)
            if normalized_payload is not None:
                return {"result": normalized_payload}
