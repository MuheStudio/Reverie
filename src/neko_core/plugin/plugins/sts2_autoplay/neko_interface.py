...
class STS2NekoInterface:
    ...
    async def set_companion_mode(self, enabled: bool) -> dict[str, Any]:
        if enabled:
            # Refresh before starting background polling so enablement has a stable
            # baseline without racing the poll loop's first tick.
            await self._service.refresh_state(trigger_sync=True)
            await self._service.refresh_state(trigger_sync=True)
        result = self._service.set_companion_mode(enabled, sync_background=not enabled)
        if enabled:
            asyncio.get_running_loop().call_soon(self._service._sync_background_polling)
        return result
