...
class STS2CompanionEvaluator:
    ...
    def _suggestion(self, *, summary_kind: str, payload: dict[str, Any], directives: dict[str, Any], trigger: str, player_operation_observation: dict[str, Any]) -> str:
        ...
        if summary_kind == "combat":
            return self._combat_voice(payload, directives=directives)
        if summary_kind == "reward":
    ...
    def _combat_voice(self, payload: dict[str, Any], *, directives: dict[str, Any]) -> str:
        player = payload.get("player") if isinstance(payload.get("player"), dict) else {}
        enemies = payload.get("enemies") if isinstance(payload.get("enemies"), list) else []
        ...
        hp_ratio = current_hp / max_hp if max_hp > 0 else 0.0
        if incoming > block or hp_ratio <= 0.45:
            named_defense = self._combat_named_card_suggestion(
                payload,
                hand=hand,
                enemies=enemies,
                prefer="defense",
            ...
            if named_defense:
                return named_defense
            named_attack = self._combat_named_card_suggestion(
                payload,
                hand=hand,
                enemies=enemies,
                prefer="attack",
    ...
    def _combat_named_card_suggestion(
        self,
        ...
        *,
        ...
        keywords = {
            "defense": ["block", "defend", "防御", "格挡"],
            "attack": ["attack", "strike", "damage", "伤害", "进攻", "输出"],
        }.get(prefer, [])

        selected_card = self._best_keyword_card(playable_cards, keywords)
        if selected_card is None and prefer == "attack":
            selected_card = self._best_enemy_pressure_card(playable_cards, enemies)
        ...
        target_name = ""
        if prefer == "attack":
            target_name = self._best_target_name(selected_card, enemies)
    ...
    def _best_keyword_card(self, cards: list[dict[str, Any]], keywords: list[str]) -> dict[str, Any] | None:
    ...
    def _best_enemy_pressure_card(self, cards: list[dict[str, Any]], enemies: list[dict[str, Any]]) -> dict[str, Any] | None:
        for card in cards:
            if self._planner._card_damage(card) > 0:
                return card
        return None

    def _best_target_name(self, card: dict[str, Any], enemies: list[dict[str, Any]]) -> str:
        if not enemies:
            return ""
        valid_target_indices = card.get("valid_target_indices") if isinstance(card.get("valid_target_indices"), list) else []
        candidate_indices: list[int] = []
        for raw_index in valid_target_indices:
            try:
                candidate_indices.append(int(raw_index))
            except Exception:
                continue
        if not candidate_indices:
            candidate_indices = list(range(len(enemies)))
        ...
        for index in candidate_indices:
            ...
            if not isinstance(enemy, dict):
                continue
            hp = self._safe_int(enemy.get("current_hp") if enemy.get("current_hp") is not None else enemy.get("hp"))
            if hp <= 0:
                continue
            score = (1 if damage > 0 and hp <= damage else 0, -hp)
            if best_score is None or score > best_score:
                best_score = score
                best_name = str(enemy.get("name") or enemy.get("enemy_id") or "").strip()
        if best_name:
            return best_name
        first_enemy = enemies[0] if isinstance(enemies[0], dict) else {}
        return str(first_enemy.get("name") or first_enemy.get("enemy_id") or "").strip()
