...
class STS2AutoplayService:
    ...
    def set_companion_mode(self, enabled: bool, *, sync_background: bool = True) -> dict[str, Any]:
    ...
    def _should_deliver_sync(self, catgirl_sync: dict[str, Any]) -> bool:
        ...
        if fingerprint and fingerprint == self._state.last_sync_fingerprint:
            ...
            same_scene = screen == self._state.last_sync_screen and summary_kind == self._state.last_sync_summary_kind
            if same_scene:
                self._state.sync_repeat_count += 1
                try:
                    self.logger.info(
                        "[sts2_push_debug] should_deliver_sync deny: duplicate scene fingerprint=%s elapsed=%s min_interval=%.3f repeat_count=%s last_screen=%s last_summary_kind=%s force=%s",
                        fingerprint,
            ...
            if self._state.last_sync_at and elapsed < min_interval:
                ...
                try:
                    self.logger.info(
                        "[sts2_push_debug] should_deliver_sync deny: duplicate fingerprint=%s elapsed=%.3f min_interval=%.3f repeat_count=%s last_screen=%s last_summary_kind=%s force=%s",
                        fingerprint,
                        elapsed,
                        min_interval,
                        self._state.sync_repeat_count,
