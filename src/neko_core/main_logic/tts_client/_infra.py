...
def _resample_audio(audio_int16: np.ndarray, src_rate: int, dst_rate: int,
    ...
    if soxr is None:
        # Fallback for environments without soxr: linear interpolation keeps the
        # module importable and preserves basic behavior for tests.
        src_len = int(audio_float.shape[0])
        if src_len <= 1:
            resampled_float = audio_float.copy()
        else:
            ...
            resampled_float = np.interp(dst_x, src_x, audio_float).astype(np.float32)
