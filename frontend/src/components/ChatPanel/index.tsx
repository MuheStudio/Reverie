...
const ChatPanel: React.FC<{
  ...
  const [input, setInput] = useState(() => loadChatDraftSync(sessionPath));
  ...
  useEffect(() => {
    ...
    setInput(loadChatDraftSync(sessionPath));
    loadChatDraft(sessionPath).then((draft) => {
  ...
  useEffect(() => {
    ...
    draftSaveTimerRef.current = setTimeout(() => {
      saveChatDraft(sessionPathRef.current, input);
    }, 300);
    return () => {
      if (draftSaveTimerRef.current) clearTimeout(draftSaveTimerRef.current);
    };
  ...
  const handleSend = useCallback(
    async (overrideText?: string) => {
      ...
      if (!overrideText) {
        setInput('');
        void clearChatDraft(sessionPathRef.current);
      }
      setSuggestedReplies([]);
        ...
        <div className={styles.chatSide}>
          <div className={styles.header}>
            ...
            <div className={styles.headerActions}>
              ...
              <button className={styles.iconBtn} onClick={onClose} title="最小化">
                <Minus size={16} />
              </button>
              <button className={styles.iconBtn} title="最大化">
                <Maximize2 size={16} />
              </button>
            </div>
          </div>
          ...
          <div className={styles.messages} data-testid="chat-messages">
            {messages.length === 0 && (
              <div className={styles.emptyState}>
                {hasUsableLLMConfig(config)
                  ? `${character.character_name} 已经准备好聊天了...`
                  : '点击齿轮配置你的模型连接'}
              </div>
            )}
            {messages.map((msg) => (
            ...
            {loading && <div className={styles.loading}>正在思考...</div>}
          ...
          <div className={styles.inputArea}>
            <textarea
              ...
              onChange={(e) => {
                ...
                setInput(nextInput);
                saveChatDraftLocalSync(sessionPath, nextInput);
              }}
  ...
  return (
    <div className={styles.overlay} data-testid="settings-overlay">
      <div className={styles.settingsModal} data-testid="settings-modal">
        <div className={styles.settingsTitle}>模型设置</div>
        ...
        <div className={styles.field}>
          <label className={styles.label}>接口地址</label>
          <input
            className={styles.fieldInput}
            value={baseUrl}
        ...
        <div className={styles.field}>
          <label className={styles.label}>自定义请求头（每行一个，Key: Value）</label>
          <textarea
            className={styles.fieldInput}
            value={customHeaders}
        ...
        <div className={styles.settingsTitle}>图像生成</div>
